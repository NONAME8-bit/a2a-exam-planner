...
logger = logging.getLogger("ExamGeneratorAgentManualLog")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",#inserts timestamp (when log happened), inserts severity (INFO, ERROR, etc.) and finally the message 
    handlers=[
        logging.StreamHandler(),                #  sends logs to console (stdout), so i see them live while running.
        logging.FileHandler("ExamGenerator_agent.log")  # persistent log file
    ]
)


REQUIRED_MCP_TOOLS = {"generate_exam_with_plan_and_rag"}
...
##Rather than mutating the old state, it creates a new dictionary with the updated values.This makes debugging and reasoning easier (each node creates a fresh version of state).
...
def make_generate_node(tools):
    async def generate_node(state: ExamState) -> ExamState:
        try:
            tool = next(t for t in tools if t.name == "generate_exam_with_plan_and_rag")
            payload = {
                "plan_json": state.get("query", ""),
                "topic": state.get("topic", None)
            }
            logger.info(f"[Generate Node] Calling tool with payload={payload}")

            result = await tool.ainvoke(payload)
            text = result if isinstance(result, str) else str(result)

            logger.info(f"[Generate Node] Tool result length={len(text)} chars")
            return {**state, "draft": text.strip(), "status": "generated"}
...
def format_output(state: ExamState) -> ExamState:
    """Format the draft into final output."""
    draft_text = state.get("draft", "")
    formatted = draft_text
    logger.info(f"[Format Node] Formatting draft (length={len(draft_text)})")
...
class ExamGeneratorAgent:
    ...
    async def _ensure_tools_loaded(self):
        if self.tools is None:
            ...
            if "generate_exam_with_plan_and_rag" not in self.tool_map:
                raise RuntimeError(
                    f"'generate_exam_with_plan_and_rag' not found. Available tools: {list(self.tool_map)}"
                )
        ...
        try:
            ...
            async for event in self.graph.astream(initial_state):
                if "node" in event:
                    ...
                    # Loop through each (key, value) pair in the event dictionary
                    for key, value in event.items():
                        # Check if the value is a dictionary and contains an "output" key
